...
func startTimerGo(t *timer) (errcode int) {
	...
	for i = 0; i < 3; i++ {
		JS_GLOBAL.Get("timer_display").Set("value", js.ValueOf("Perfection🔥"))
		//these calls, combined, MUST take at least 7ms. I'm trying to counter it.
		time.Sleep(time.Millisecond * time.Duration(332))
		JS_GLOBAL.Get("timer_display").Set("value", js.ValueOf("Perfection🔥🔥"))
		time.Sleep(time.Millisecond * time.Duration(332))
		JS_GLOBAL.Get("timer_display").Set("value", js.ValueOf("Perfection🔥🔥🔥"))
		time.Sleep(time.Millisecond * time.Duration(332))
